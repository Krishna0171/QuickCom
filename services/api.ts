...
class SupabaseApiService {
  ...
  async initializeDatabase(): Promise<{ success: boolean; error?: string }> {
    if (!this.supabase) return { success: false, error: 'Supabase credentials missing.' };
  ...
  async runAutoSetup(): Promise<boolean> {
    ...
    try {
      // Attempt to run the schema via a helper RPC if the user has it
...
$$ LANGUAGE plpgsql SECURITY DEFINER;`;
  ...
  async toggleFavorite(productId: string): Promise<string[]> {
    ...
    if (!isFav) {
      await this.supabase.from(STORES.FAVORITES).insert({ 
